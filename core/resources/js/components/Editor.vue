<template>
    <div class="fixed z-30 top-0 right-0 bottom-0 w-[700px] shadow-md p-5 bg-teal-100 max-h-full overflow-auto">
        <div class="flex flex-col min-h-full">
            <div class="flex-grow">
                <h1 class="text-lg mb-4">Edit <strong>{{ component.user.name }}</strong></h1>
                <div class="space-y-4">
                    <component :is="field.type" v-for="(field,key) in component.user.fields" v-model="form[key]"
                               :field="field"></component>
                </div>
            </div>
            <div class="flex gap-4 mt-4 flex-shrink-0">
                <button class="bg-black cursor-pointer text-white px-4 py-2 font-bold" @click.prevent="save">Save
                </button>
                <button class="bg-gray-300 cursor-pointer px-4 py-2 font-bold" @click.prevent="model = false">Cancel
                </button>
            </div>
        </div>
    </div>
